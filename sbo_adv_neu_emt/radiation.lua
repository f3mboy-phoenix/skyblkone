...
local function dmg_abm(pos, node)
    local strength = (minetest.get_item_group(node.name, 'heavy_radioactive') * 20)
...
if minetest.settings:get_bool('enable_damage') then
    minetest.register_abm({
        ...
        nodenames = { 'group:heavy_radioactive' },
        interval = 1,
        chance = 1,
        action = dmg_abm,
    })
...
-- RADON!!!
core.register_node('sbz_chem:heavy_radon', {
    description = 'radon',
    drawtype = 'glasslike',
    paramtype = 'light',
    drop = '',
    tiles = { 'heavy_radon.png' },
    light_source = 10,
...
do
    ...
    local function radon_move_filter(pos)
        ...
        if core.get_item_group(nn, 'heavy_radioactive') > 0 then
    ...
    core.register_abm({
        label = 'Heavy Radon gas move',
        nodenames = { 'sbz_chem:heavy_radon' },
...
core.register_abm({
    label = 'Heavy Radon gas/water spawn (by group:heavy_radioactive)',
    nodenames = { 'group:heavy_radioactive' },
    interval = 1,
    chance = 1,
    action = function(spos, node)
        local radioactive_group = core.get_item_group(node.name, 'heavy_radioactive')
        local num_radon = 0
        ...
        for i = 1, num_radon do
            ...
            if data.type == 'air' then
                node_to_place = 'sbz_chem:heavy_radon'
            elseif data.type == 'water' then
                node_to_place = 'sbz_chem:heavy_radioactive_water'
            end
...
core.register_abm({
    ...
    nodenames = { 'sbz_chem:heavy_radioactive_water' },
    neighbors = { 'air', 'sbz_resources:water_source', 'sbz_chem:heavy_radioactive_water' },
    interval = 2,
    chance = 1,
    action = function(spos, node)
        ...
        then
            ...
            core.set_node(uppos, { name = 'sbz_chem:heavy_radon' })
            return
        end
