...
local function try_linking(pos, meta)
    local nodes = minetest.find_nodes_in_area(vector.subtract(pos, offset), vector.add(pos, offset),
        { "sbo_reactors:resium_reactor_core_off", "sbo_reactors:resium_reactor_core_on" }, true)
    local firstpos
    if nodes["sbo_reactors:resium_reactor_core_off"] ~= nil then
        firstpos = nodes["sbo_reactors:resium_reactor_core_off"][1]
    elseif nodes["sbo_reactors:resium_reactor_core_on"] ~= nil then
        firstpos = nodes["sbo_reactors:resium_reactor_core_on"][1]
    else
...
minetest.register_node("sbo_reactors:resium_reactor_shell", {
...
minetest.register_craft {
    output = "sbo_reactors:resium_reactor_shell",
    recipe = {
...
minetest.register_node("sbo_reactors:resium_reactor_glass", {
...
minetest.register_craft {
    output = "sbo_reactors:resium_reactor_glass",
    recipe = {
        { "sbz_power:simple_charged_field", "sbo_resium:glass",    "sbz_power:simple_charged_field" },
        { "sbo_resium:glass",  "sbo_reactors:resium_reactor_shell", "sbo_resium:glass" },
        { "sbz_power:simple_charged_field", "sbo_resium:glass",    "sbz_power:simple_charged_field" }
    }
}
...
minetest.register_node("sbo_reactors:resium_reactor_item_input", {
...
minetest.register_craft {
    output = "sbo_reactors:resium_reactor_item_input",
    recipe = {
        { "pipeworks:tube_1", "sbz_resources:retaining_circuit", "sbo_reactors:resium_reactor_shell" }
    }
}

sbz_api.register_stateful("sbo_reactors:resium_reactor_core", {
...
minetest.register_craft {
    output = "sbo_reactors:resium_reactor_core",
    recipe = {
        { "sbo_reactors:resium_reactor_shell", "sbz_meteorites:neutronium",         "sbo_reactors:resium_reactor_shell", },
        { "sbz_meteorites:neutronium",        "sbo_reactors:extrosim_reactor_core", "sbz_meteorites:neutronium", },
        { "sbo_reactors:resium_reactor_shell", "sbz_meteorites:neutronium",         "sbo_reactors:resium_reactor_shell", }
    }
}
...
minetest.register_node("sbo_reactors:resium_reactor_infoscreen", {
    ...
    on_rightclick = function(pos)
        ...
        if linkedname ~= "sbo_reactors:resium_reactor_core_on" and linkedname ~= "sbo_reactors:resium_reactor_core_off" then
            if not try_linking(pos, meta) then
                meta:set_string("infotext", "No reactor nearby")
                meta:set_int("linked", 0)
                meta:set_string("formspec", "")
...
minetest.register_craft {
    output = "sbo_reactors:resium_reactor_infoscreen",
    recipe = {
        { "sbo_reactors:resium_reactor_glass", "sbo_reactors:resium_reactor_shell", "sbz_power:connector_off" }
    }
}

sbz_api.register_generator("sbo_reactors:resium_reactor_power_port", {
...
minetest.register_craft {
    output = "sbo_reactors:resium_reactor_power_port",
    recipe = {
        { "sbz_power:power_pipe", "sbo_reactors:resium_reactor_shell", "sbz_power:simple_charged_field" }
    }
}

minetest.register_node("sbo_reactors:resium_reactor_coolant_port", {
...
minetest.register_craft {
    output = "sbo_reactors:resium_reactor_coolant_port",
    recipe = {
        { "sbz_power:fluid_pipe", "sbz_power:fluid_tank", "sbo_reactors:resium_reactor_shell" }
    }
}
...
local function core_tick(pos)
    ...
    for x = iter_start_pos.x, iter_start_pos.x + 6 do
        for y = iter_start_pos.y, iter_start_pos.y + 6 do
            for z = iter_start_pos.z, iter_start_pos.z + 6 do
                ...
                if node == "sbo_reactors:resium_reactor_power_port" then
                ...
                elseif node == "sbo_reactors:resium_reactor_infoscreen" then
                    if nodes.info == nil then
                        nodes.info = vec
                    else
                        err = "You can't have more than 1 infoscreen"
                    end
                elseif node == "sbo_reactors:resium_reactor_item_input" then
                    if nodes.resium == nil then
                        nodes.resium = vec
                    else
                        err = "You can't have more than 1 Resium Input"
                    end
                elseif node == "sbo_reactors:resium_reactor_coolant_port" then
    ...
    if not err then
        ...
        minetest.registered_nodes["sbo_reactors:resium_reactor_infoscreen"].on_reactor_update(nodes.info)
...
minetest.register_abm({
    nodenames = { "sbo_reactors:resium_reactor_core_on" },
    action = core_tick,
    interval = 1,
    chance = 1,
})
...
mesecon.register_on_mvps_move(function(moved_nodes)
    for i = 1, #moved_nodes do
        ...
        if moved_node.node.name == "sbo_reactors:resium_reactor_power_port" then
        ...
        elseif moved_node.node.name == "sbo_reactors:resium_reactor_infoscreen" then
            local meta = minetest.get_meta(moved_node.pos)
...
unified_inventory.add_category_item('reactors', "sbo_reactors:resium_reactor_shell")
unified_inventory.add_category_item('reactors', "sbo_reactors:resium_reactor_glass")
unified_inventory.add_category_item('reactors', "sbo_reactors:resium_reactor_item_input")
unified_inventory.add_category_item('reactors', "sbo_reactors:resium_reactor_core_off")
unified_inventory.add_category_item('reactors', "sbo_reactors:resium_reactor_infoscreen")
unified_inventory.add_category_item('reactors', "sbo_reactors:resium_reactor_power_port")
unified_inventory.add_category_item('reactors', "sbo_reactors:resium_reactor_coolant_port")

core.register_alias("sbo_resium_reactor:reactor_shell", "sbo_reactors:resium_reactor_shell")
core.register_alias("sbo_resium_reactor:reactor_glass", "sbo_reactors:resium_reactor_glass")
core.register_alias("sbo_resium_reactor:reactor_item_input", "sbo_reactors:resium_reactor_item_input")
core.register_alias("sbo_resium_reactor:reactor_core_off", "sbo_reactors:resium_reactor_core_off")
core.register_alias("sbo_resium_reactor:reactor_core_on", "sbo_reactors:resium_reactor_core_on")
core.register_alias("sbo_resium_reactor:reactor_infoscreen", "sbo_reactors:resium_reactor_infoscreen")
core.register_alias("sbo_resium_reactor:reactor_power_port", "sbo_reactors:resium_reactor_power_port")
core.register_alias("sbo_resium_reactor:reactor_coolant_port", "sbo_reactors:resium_reactor_coolant_port")
...
sbo_api.quests.register_to("SBO: Other infos",{
    ...
    text =
        [[Resium Reactors produce 1MCj. Setup like Emittrium Reactors except 7x7x7]],
})
