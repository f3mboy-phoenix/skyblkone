...
sbo_computer.node_config = {}

local have_technic = minetest.get_modpath("technic")
local have_generator = minetest.get_modpath("power_generators")

local function on_construct(pos)
	sbo_computer.mtos_cache:free(pos)
	local mtos = sbo_computer.os_get(pos)
	local node = minetest.get_node(pos)
	local hwdef = sbo_computer.node_config[node.name]
...
local function on_punch(pos, node, puncher)
	...
	if is_compatible then
		...
		sbo_computer.mtos_cache:sync_and_free(mtos)
		for k,v in pairs(sbo_computer.os_get(mtos.pos)) do
			mtos[k] = v
		end
	...
	if hwdef.next_node then
		local hwdef_next = sbo_computer.node_config[hwdef.next_node]
	...
	if (supply<demand) and (demand>0) then
		...
		if power_off then
			...
			if hwdef.power_off_node then
				local hwdef_next = sbo_computer.node_config[hwdef.power_off_node]
				if hwdef_next.hw_state then
...
function sbo_computer.register_hardware(name, hwdef)
	...
	for idx, variant in ipairs(hwdef.sequence) do
		...
		-- Defaults
		merged_hwdef.hw_capabilities =	merged_hwdef.hw_capabilities or {"hdd", "usb", "net", "liveboot"}
		sbo_computer.node_config[nodename] = merged_hwdef
	end
