...
sbo_computer.os_version_attr.default = sbo_computer.os_version_attr['10.00']

-----------------------------------------------------
-- Hard-Coded supported monochrome colors
-----------------------------------------------------
sbo_computer.supported_textcolors = {
	GREEN = "#00FF33",
	AMBER = "#FFB000",
	WHITE = "#FFFFFF",
}
...
sbo_computer.mtos_cache = mtos_cache
...
sbo_computer.class_lib.os = os_class
...
function os_class:swap_node(new_node_name)
	...
	if new_node_name then
		...
		self.hwdef = sbo_computer.node_config[self.node.name]
	end
...
function os_class:power_on(new_node_name)
	...
	for k,v in pairs(sbo_computer.os_get(self.pos)) do
		self[k] = v
	end
...
function os_class:get_theme(theme)
	...
	return sbo_computer.get_theme(theme)
end

-- Set current theme
function os_class:set_theme(theme)
	if sbo_computer.themes[theme] then
		if self.sysdata then
...
function os_class:get_os_attr()
	...
	for k, v in pairs(sbo_computer.os_version_attr[os_version]) do
		os_attr[k] = v
	end
...
function os_class:is_app_compatible(name)
	local app_def = sbo_computer.apps[name]
	if not app_def then
		return false
	end
...
function os_class:is_theme_compatible(name)
	local theme_def = sbo_computer.themes[name]
...
function os_class:get_app(name)
	local template = sbo_computer.apps[name]
	if not template then
		return
	end
	local app = setmetatable({}, sbo_computer.class_lib.app)
...
-----------------------------------------------------
function sbo_computer.os_get(pos)
	...
	self.bdev = sbo_computer.get_bdev_handler(self)
