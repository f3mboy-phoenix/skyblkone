...
function sbo_computer.register_theme(name, def)
	sbo_computer.themes[name] = def
end

-- load all themes
local theme_path = minetest.get_modpath('sbo_computer')..'/themes/'
...
function sbo_computer.get_theme(theme_name)
	theme_name = theme_name or "Freedom"
	local self = setmetatable({}, theme_class)
	for k, v in pairs(sbo_computer.themes.default) do
		self[k] = v
	end
	if theme_name and sbo_computer.themes[theme_name] then
		for k,v in pairs(sbo_computer.themes[theme_name]) do
			self[k] = v
		end
