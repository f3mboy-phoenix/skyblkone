...
local function trigger_queue(app, mtos)
	...
	if mtos.sysdata.print_progress >= 5 then
		...
		local stack = ItemStack("sbo_computer:printed_paper")
...
sbo_computer.register_app("printer_launcher", {
...
local function get_printer_info(pos)
	local hw_os = sbo_computer.os_get(pos)
	local printer
	if hw_os then
		printer = {
				pos = pos,
...
sbo_computer.register_view("printer:app", {
	app_info = "Print file",
	...
	receive_fields_func = function(app, mtos, sender, fields)
		...
		if fields.print and sysstore.selected_printer then
			local hw_os = sbo_computer.os_get(sysstore.selected_printer.pos)
