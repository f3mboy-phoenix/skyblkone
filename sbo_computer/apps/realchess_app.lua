...
function realchess.move(data, pos, from_list, from_index, to_list, to_index, _, player)
	...
	local pieceFrom = inv:get_stack(from_list, from_index):get_name():sub(13) --:sub(8) cuts "sbo_computer:"
	local pieceTo = inv:get_stack(to_list, to_index):get_name():sub(13)    --:sub(8) cuts "sbo_computer:"
			...
			if from_y - 1 == to_y then
				if from_x == to_x then
					...
					elseif to_index >= 1 and to_index <= 8 then
						inv:set_stack(from_list, from_index, "sbo_computer:realchess_queen_white")
					end
			...
			if from_y + 1 == to_y then
				if from_x == to_x then
					...
					elseif to_index >= 57 and to_index <= 64 then
						inv:set_stack(from_list, from_index, "sbo_computer:realchess_queen_black")
					end
				elseif from_x - 1 == to_x or from_x + 1 == to_x then
		...
		if thisMove == "white" then
			if from_y == 7 and to_y == 7 then
				if to_x == 1 then
					...
					if data.castlingWhiteL == 1 and idx57 == "sbo_computer:realchess_rook_white_1" then
						...
						inv:set_stack(from_list, 59, "sbo_computer:realchess_rook_white_1")
						check = false
					end
				elseif to_x == 6 then
					local idx64 = inv:get_stack(from_list, 64):get_name()

					if data.castlingWhiteR == 1 and idx64 == "sbo_computer:realchess_rook_white_2" then
						for i = from_index + 1, 63 do
							if inv:get_stack(from_list, i):get_name() ~= "" then
								return 0
							end
						end
						inv:set_stack(from_list, 62, "sbo_computer:realchess_rook_white_2")
		...
		elseif thisMove == "black" then
			if from_y == 0 and to_y == 0 then
				if to_x == 1 then
					...
					if data.castlingBlackL == 1 and idx1 == "sbo_computer:realchess_rook_black_1" then
						...
						inv:set_stack(from_list, 3, "sbo_computer:realchess_rook_black_1")
						check = false
					end
				elseif to_x == 6 then
					local idx8 = inv:get_stack(from_list, 1):get_name()

					if data.castlingBlackR == 1 and idx8 == "sbo_computer:realchess_rook_black_2" then
						for i = from_index + 1, 7 do
							if inv:get_stack(from_list, i):get_name() ~= "" then
								return 0
							end
						end
						inv:set_stack(from_list, 6, "sbo_computer:realchess_rook_black_2")
...
local function register_piece(name, count)
	for _, color in pairs({ "black", "white" }) do
		if not count then
			minetest.register_craftitem("sbo_computer:realchess_" .. name .. "_" .. color, {
			...
			for i = 1, count do
				minetest.register_craftitem("sbo_computer:realchess_" .. name .. "_" .. color .. "_" .. i, {
					description = color:gsub("^%l", string.upper) .. " " .. name:gsub("^%l", string.upper),
					inventory_image = "laptop_realchess_" .. name .. "_" .. color .. ".png",
					stack_max = 1,
					groups = { not_in_creative_inventory = 1 }
...
sbo_computer.register_app("realchess", {
	app_name = "Realchess",
	...
	formspec_func = function(app, mtos)
		...
		if not data.init_done then
			...
			inv:set_list("board", {
				"sbo_computer:realchess_rook_black_1",
				"sbo_computer:realchess_knight_black_1",
				"sbo_computer:realchess_bishop_black_1",
				"sbo_computer:realchess_queen_black",
				"sbo_computer:realchess_king_black",
				"sbo_computer:realchess_bishop_black_2",
				"sbo_computer:realchess_knight_black_2",
				"sbo_computer:realchess_rook_black_2",
				"sbo_computer:realchess_pawn_black_1",
				"sbo_computer:realchess_pawn_black_2",
				"sbo_computer:realchess_pawn_black_3",
				"sbo_computer:realchess_pawn_black_4",
				"sbo_computer:realchess_pawn_black_5",
				"sbo_computer:realchess_pawn_black_6",
				"sbo_computer:realchess_pawn_black_7",
				"sbo_computer:realchess_pawn_black_8",
				'', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '',
				'', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '',
				"sbo_computer:realchess_pawn_white_1",
				"sbo_computer:realchess_pawn_white_2",
				"sbo_computer:realchess_pawn_white_3",
				"sbo_computer:realchess_pawn_white_4",
				"sbo_computer:realchess_pawn_white_5",
				"sbo_computer:realchess_pawn_white_6",
				"sbo_computer:realchess_pawn_white_7",
				"sbo_computer:realchess_pawn_white_8",
				"sbo_computer:realchess_rook_white_1",
				"sbo_computer:realchess_knight_white_1",
				"sbo_computer:realchess_bishop_white_1",
				"sbo_computer:realchess_queen_white",
				"sbo_computer:realchess_king_white",
				"sbo_computer:realchess_bishop_white_2",
				"sbo_computer:realchess_knight_white_2",
				"sbo_computer:realchess_rook_white_2"
			})
		end
		local formspec =
