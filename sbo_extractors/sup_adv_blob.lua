...
sbz_api.register_machine("sbo_extractors:adv_antiblob_ext", {
    description =
    "Super Advanced Blob Extractor",
    tiles = { "sbo_supadv_blob_ext.png" },
    groups = { matter = 1, sbz_machine = 1, pipe_connects = 1 },
    sunlight_propagates = true,
    walkable = true,
    on_rightclick = function(pos, node, player, pointed_thing)
        local player_name = player:get_player_name()
        minetest.show_formspec(player_name, "sbz_power:sbo_supadv_blob_ext",
            "formspec_version[7]" ..
            "size[8.2,9]" ..
    ...
    action = function(pos, node, meta)
        local inv = meta:get_inventory()
        local itemstack = ItemStack("sbz_resources:matter_blob")
        itemstack:set_count(20)
...
sbo_api.quests.on_craft["sbo_extractors:adv_antiblob_ext"] = "Super Advanced Blob Extractor"
sbo_api.quests.register_to("Questline: Resium",{
    type = "quest",
    title = "Super Advanced Blob Extractor",
    text = [[Useful for automation. Creates 20 Matter Blobs per second. Consumes 500 Power!]],
    requires = { "Cement Mixer", }
})
core.register_alias("sbo_supadv_blob_ext:extractor", "sbo_extractors:adv_antiblob_ext")
