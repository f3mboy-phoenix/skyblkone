...
local function try_linking(pos, meta)
    local nodes = minetest.find_nodes_in_area(vector.subtract(pos, offset), vector.add(pos, offset),
        { "sbo_extrosim_reactor:reactor_core_off", "sbo_extrosim_reactor:reactor_core_on" }, true)
    local firstpos
    if nodes["sbo_extrosim_reactor:reactor_core_off"] ~= nil then
        firstpos = nodes["sbo_extrosim_reactor:reactor_core_off"][1]
    elseif nodes["sbo_extrosim_reactor:reactor_core_on"] ~= nil then
        firstpos = nodes["sbo_extrosim_reactor:reactor_core_on"][1]
    else
...
minetest.register_node("sbo_extrosim_reactor:reactor_shell", {
...
minetest.register_node("sbo_extrosim_reactor:reactor_glass", {
...
minetest.register_node("sbo_extrosim_reactor:reactor_item_input", {
...
minetest.register_craft {
    output = "sbo_extrosim_reactor:reactor_item_input",
    recipe = {
        { "pipeworks:tube_1", "sbz_resources:retaining_circuit", "sbo_extrosim_reactor:reactor_shell" }
    }
}

sbz_api.register_stateful("sbo_extrosim_reactor:reactor_core", {
...
minetest.register_node("sbo_extrosim_reactor:reactor_infoscreen", {
    ...
    on_rightclick = function(pos)
        ...
        if linkedname ~= "sbo_extrosim_reactor:reactor_core_on" and linkedname ~= "sbo_extrosim_reactor:reactor_core_off" then
            if not try_linking(pos, meta) then
                meta:set_string("infotext", "No reactor nearby")
                meta:set_int("linked", 0)
                meta:set_string("formspec", "")
...
minetest.register_craft {
    output = "sbo_extrosim_reactor:reactor_infoscreen",
    recipe = {
        { "sbo_extrosim_reactor:reactor_glass", "sbo_extrosim_reactor:reactor_shell", "sbz_power:connector_off" }
    }
}

sbz_api.register_generator("sbo_extrosim_reactor:reactor_power_port", {
...
minetest.register_craft {
    output = "sbo_extrosim_reactor:reactor_power_port",
    recipe = {
        { "sbz_power:power_pipe", "sbo_extrosim_reactor:reactor_shell", "sbz_power:simple_charged_field" }
    }
}
...
minetest.register_craft {
    output = "sbo_extrosim_reactor:reactor_coolant_port",
    recipe = {
        { "sbz_power:fluid_pipe", "sbz_power:fluid_tank", "sbo_extrosim_reactor:reactor_shell" }
    }
}
...
local function core_tick(pos)
    ...
    for x = iter_start_pos.x, iter_start_pos.x + 4 do
        for y = iter_start_pos.y, iter_start_pos.y + 4 do
            for z = iter_start_pos.z, iter_start_pos.z + 4 do
                ...
                if node == "sbo_extrosim_reactor:reactor_power_port" then
                    if nodes.power == nil then
                        nodes.power = vec
                    else
                        err = "You can't have more than 1 power port"
                    end
                    --                elseif node == "sbo_extrosim_reactor:reactor_core" then
                ...
                elseif node == "sbo_extrosim_reactor:reactor_infoscreen" then
                    if nodes.info == nil then
                        nodes.info = vec
                    else
                        err = "You can't have more than 1 infoscreen"
                    end
                elseif node == "sbo_extrosim_reactor:reactor_item_input" then
                    if nodes.extrosim == nil then
                        nodes.extrosim = vec
                    else
                        err = "You can't have more than 1 Extrosim Input"
                    end
                elseif node == "sbo_extrosim_reactor:reactor_coolant_port" then
    ...
    if not err then
        ...
        minetest.registered_nodes["sbo_extrosim_reactor:reactor_infoscreen"].on_reactor_update(nodes.info)
...
minetest.register_abm({
    nodenames = { "sbo_extrosim_reactor:reactor_core_on" },
    action = core_tick,
    interval = 1,
    chance = 1,
})
...
mesecon.register_on_mvps_move(function(moved_nodes)
    for i = 1, #moved_nodes do
        ...
        if moved_node.node.name == "sbo_extrosim_reactor:reactor_power_port" then
        ...
        elseif moved_node.node.name == "sbo_extrosim_reactor:reactor_infoscreen" then
            local meta = minetest.get_meta(moved_node.pos)
