...
local use_invisibility = core.get_modpath("sbo_invisibility")
...
mobs = {
	...
	node_snow = has("sbz_planets:snow") or "air",
	node_dirt = has("sbz_bio:dirt") or "mobs:fallback_node"
}
mobs.fallback_node = mobs.node_dirt

-- load compatibility check function

dofile(core.get_modpath("sbo_mobs") .. "/compatibility.lua")
...
core.register_entity("sbo_mobs:_pos", {
	initial_properties = {
...
function mob_class:go_to(pos)
	...
	local obj = core.add_entity(pos, "sbo_mobs:_pos")
...
function mob_class:do_states(dtime)
	...
	if is_node_dangerous(self, self.standing_in) then
		...
		local lp = core.find_nodes_in_area_under_air(
				...
				{x = s.x + 7, y = s.y + 1.0, z = s.z + 7},
				{"group:matter", "group:antimatter", "group:explody", "group:solid"})
					...
					if not self.custom_attack or self:custom_attack(self, p) then
						...
						if self:line_of_sight(p2, s2) then
							...
							core.after(0.2, function()
								...
								local hp = self.attack and self.attack:get_hp()

								if hp then self.attack:set_hp(hp - self.damage) end
							end)
						end
					end
				end
...
function mobs:spawn(def)
	...
	mobs:spawn_specific(
		def.name,
		def.nodes or {"group:soil", "group:matter"},
...
function mobs:capture_mob(
	...
	and tool:get_name() ~= "sbo_mobs:lasso" then
		return false
	end
...
--print ("====", self.hornytimer)
	...
	if item:get_name() == "sbo_mobs:nametag"
...
core.register_on_player_receive_fields(function(player, formname, fields)
	...
	if formname == "mobs_nametag" and fields.name and fields.name ~= "" then
		...
		if item:get_name() ~= "sbo_mobs:nametag" then return end
